...
The current evaluation is source-level, not exact chunk-level. A retrieval run gets credit when the top K results contain the expected source IDs, such as `gdpr_en` or `eu_ai_act_en`.
...
| Retriever | Answerable Questions | Recall@5 | Hit@5 | MRR |
| --- | ---: | ---: | ---: | ---: |
| BM25 | 32 | 0.641 | 0.938 | 0.769 |
...
- Source-level labels are useful but too coarse for final production evaluation.
- Refusal questions need answer-level scoring, not only retrieval inspection.

## Next Improvements

1. Add exact expected citation labels for the highest-value questions.
2. Compare stronger embedding models such as `BAAI/bge-m3`.
