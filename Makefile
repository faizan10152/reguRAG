...
eval-answers-ollama:
	$(HF_LOCAL_ENV) uv run --extra rag regurag eval-answers --chunks data/processed/chunks.jsonl --questions data/eval/golden_questions_v1.jsonl --retriever hybrid-rerank --qdrant-path .qdrant/bge-m3 --model BAAI/bge-m3 --reranker-model cross-encoder/mmarco-mMiniLMv2-L12-H384-v1 --top-k 5 --candidate-k 20 --llm-model "$(OLLAMA_MODEL)" --llm-api-base "$(OLLAMA_API_BASE)" --disable-json-mode --question-ids "$(ANSWER_EVAL_IDS)" --output-md reports/answer_eval_ollama_latest.md --output-json reports/answer_eval_ollama_latest.json
